...
In this case you have to compare the subgroups manually by <Compare
Subgroups>. Note that this can mean large computations, especially if
...
right corner, then  release  the mouse  button), and   choose <Compare
Subgroups> from the ``Subgroups'' menu.  A few vertices will disappear
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\Section{A Partial Subgroup Lattice of an Infinite Space Group}

This section describes an investigation of an infinite space group
provided by the CRYST and CRYSTCAT share packages. Here you will see
that subgroups of finite index and of finite size can occur at the
same time in a graphic subgroup lattice of an infinite group. Note
that you have to install these share package to try this example.
...
gap> mm := m{[1..3]};
#I  InsertVertices from GAP (G) --> (2,3,4)
\endtt
...
Calculate subgroups of infinite index but with finite size:
...
basis := [  ] >
gap> www := WyckoffStabilizer(ww);
<matrix group of size 8 with 3 generators>
\endtt

Insert this subgroup into the lattice by selecting
`InsertSubgroups from GAP' from the <Subgroups> menu. It will be
inserted in the level for groups of size 8.

%You will notice
%that these vertices are placed into ``infinity'' levels, because
%{\GAP} claims not to be able to calculate the (finite) size of these
%subgroups. Open an information menu by clicking with the right mouse
%button on one of the new vertices. There you see that actually the
%`Size' is already known because it was calculated during the index
%calculation. If you now click on any entry in this information menu,
%{\XGAP} will notice this and inform you, that it puts the vertex into
%a new level for finite size. Note that internally the vertex is
%deleted and newly created, so it changes its name. All information
%currently known is however preserved.
