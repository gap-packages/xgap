...
If you now select vertex $5$ and choose `Centralizer' from the
<Subgroups> menu, you will get a new vertex representing a group with
infinite index *and* infinite size. Such a vertex is placed into a
level on its own, which is marked by `[ infinity, <n>]' where <n> is
replaced with subsequent natural numbers (see section "levelsintro"
for details about levels).
