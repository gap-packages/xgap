...
In order to *select* vertex $G$ place the pointer inside the diamond around
...
indices. For those subgroups with an index greater or equal to
`GGLLimitForIsNormalCalc' (currently set to $1000$, see
"GGLLimitForIsNormalCalc") {\XGAP} does not calculate whether they are
normal in $G$, because that would require a coset table. They are
drawn as vertices with a rectangular shape. Note that these groups are
only represented as kernels of epimorphisms within {\GAP}.
...
finitely presented group that is defined by a coset table or as kernel of
an epimorphism is *not* itself a finitely presented group. This means, that 
certain algorithms can not be applied. Use `IsomorphismFpGroup' (see
"ref:IsomorphismFpGroup" in the {\GAP} reference manual) to calculate a
finitely presented group and an isomorphism onto it.
